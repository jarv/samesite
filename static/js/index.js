...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  // node_modules/@alpine-collective/toolkit-interval/dist/module.esm.js
  function src_default2(Alpine2) {
    Alpine2.magic("interval", () => {
      return function(...parameters) {
        ...
        const loop2 = () => {
          ...
          autoIntervalLoop = setTimeout(() => {
            ...
            forceInterval ? loop2() : requestAnimationFrame(loop2);
          }, next);
        };
        Alpine2.effect(() => {
  ...
  module_default.data("cookie", () => ({
    ...
    deleteCookies() {
      Object.getOwnPropertyNames(api.get()).forEach((c) => {
        if (!c.includes("sameSite")) {
          return;
        }
        try {
          const attr = JSON.parse(atob(api.get(c) || ""));
          api.remove(c, attr);
        } catch (error2) {
    ...
    updateWithContent(url, className, method, isSecure, isPartitioned) {
      ...
      fetch(url, options).then((response) => {
        ...
        console.error(`Error updating elements: ${error2.message}`);
      });
    }
  }));
